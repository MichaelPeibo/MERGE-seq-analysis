...
###############################################################################################################################
MULTIseq.align.v2 <- function(readTable, cellIDs, ref) {
  require(stringdist)
  ...
  for (bucket in 1:length(cellIDs_bucketed)) {
    ...
    for (cell in cellIDs.temp) {
      ## looping five barcodes，count UMI seperately
      for (n in 1:5) {
        ...
        ##更改HD
        ## Find tags with HD <= 2 from any reference tag
        tag.dists <- stringdist(a=tags, b=ref[n],method = 'hamming')
        #tag.hds <- apply(tag.dists, 1, min)
        tag.ind <- which(tag.dists <= 2)
        ...
        if (length(umi.ind) == 1) {
          ...
          #bar.table[cell,"nUMI"] <- length(umi.ind)
            bar.table[cell,n] <- length(which(tag.dists[n] <= 2))
        ...
        if (length(umi.ind) > 1) {
          ...
          #bar.table[cell,"nUMI"] <- length(umi.ind)
            bar.table[cell,n] <- length(which(tag.dists <= 2))
          }
      }
      bar.table[cell, "nUMI"] <- sum(bar.table[cell, 1:5])
    }
  }
