...
setwd('/Users/peiboxu/Desktop/merge-seq analysis/sa_revision_new_thre/')
...
genes.use=setdiff(rownames(mda),barcode_genes)
mda=subset(mda,features=genes.use)

sub_meta = meta %>% filter(DMS_valid=='Others' & AI_valid=='Others' & LH_valid=='Others' &
                  BLA_valid=='Others' & MD_valid=='Others')
mda=pfc.bar.sub
pfc.sub.meta=mda@meta.data
pfc.sub.meta.new=meta[rownames(pfc.sub.meta),]
### calculate DEGs without barcodes genes###
Idents(mda)=pfc.sub.meta.new$DMS_valid
...
#### DMS ####
Idents(mda)=meta$DMS_valid
table(Idents(mda))
dms_markers=FindMarkers(mda,ident.1="DMS_valid",ident.2="Others",
                        logfc.threshold = 0.25, min.pct = 0.1,
                        test.use = "MAST", 
                         verbose = TRUE, only.pos = F)
head(dms_markers,n=20)
write.csv(dms_markers,file='dms_markers_3183.csv')
...
#### AI ####
Idents(mda)=meta$AI_valid
table(Idents(mda))
ai_markers=FindMarkers(mda,ident.1="AI_valid",ident.2="Others",
                        logfc.threshold = 0.25, min.pct = 0.1,
                        test.use = "MAST", 
                        verbose = TRUE, only.pos = F)
...
#### MD ####
Idents(mda)=meta$MD_valid
table(Idents(mda))
md_markers=FindMarkers(mda,ident.1="MD_valid",ident.2="Others",
                       logfc.threshold = 0.25, min.pct = 0.1,
                       test.use = "MAST", 
                       verbose = TRUE, only.pos = F)
...
#### LH ####
Idents(mda)=meta$LH_valid
table(Idents(mda))
lh_markers=FindMarkers(mda,ident.1="LH_valid",ident.2="Others",
                       logfc.threshold = 0.25, min.pct = 0.1,
                       test.use = "MAST", 
                       verbose = TRUE, only.pos = F)
...
#### BLA ####
Idents(mda)=meta$BLA_valid
table(Idents(mda))
bla_markers=FindMarkers(mda,ident.1="BLA_valid",ident.2="Others",
                       logfc.threshold = 0.25, min.pct = 0.1,
                       test.use = "MAST", 
                       verbose = TRUE, only.pos = F)
