PieDonut_modified_v1=function(data,mapping,center_label,which_target,
        ...
        if(which_target %in% c(1,2)) {mainCol=c('#7fc97f', '#beaed4', '#fdc086', '#386cb0', '#f0027f', '#bf5b17','#666666')
                } else {mainCol=c('#7fc97f', '#beaed4', '#fdc086', '#f0027f', '#bf5b17','#666666')}
        ...
        if(!is.null(donuts)){
                ...
                df3
                #print(levels(df3$cluster))
                del=which(df3$Freq==0)
                del
                #if(length(del)>0) subColor<-subColor[-del]
                #print(subColor)
                #df3$cluster <- factor(df3$cluster, levels = levels(exn_meta$cluster), ordered = TRUE)
                #df3$layer_v1 <- factor(df3$layer_v1, levels = levels(exn_meta$layer_v1), ordered = TRUE)
        ...
        if(showPieName) p1<-p1+annotate("text",x=0,y=0,label=center_label,size=titlesize,family=family)
        ...
        if(!is.null(donuts)){
                ...
                if(explodeDonut) {
                        ...
                        p3<-p+geom_arc_bar(aes_string(x0 = "x", y0 = "y", r0 = as.character(r1),
                                                      r = as.character(r2), start="start1",end="end1",
                                                      fill = "no"),alpha=donutAlpha,color=mainCol,
                                           data = df3)
                }
