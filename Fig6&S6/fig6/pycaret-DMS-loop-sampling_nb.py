...
for x in sample_trials:
    df = pd.read_csv('./sampling_results/var5000_DMS_valid_binary_normdata_1000cells_%d.csv' % (x))
    for n in num_vars:
        AUC = []
        Accuracy = []
        Recall = []
        ...
        metrics_data.to_csv(f"./dms_results_nb/DMS_var_model_results_{n}_genes_metrics_{x}.csv", index=False)
